...
<?php
echo link_to_function($gadgetConfig[$gadget->getName()]['caption']['zh_CN'], 'showModalOnParent(\''.url_for('design/editGadget?id='.$gadget->getId()).'\')');
?>
</div>
<?php endforeach; ?>
<?php endif; ?>
<div class="emptyGadget">
<?php echo link_to_function(__('增加小部件'), 'showModalOnParent(\''.url_for('design/addGadget?type='.$type).'\')') ?>
</div>
</div>
