...
function mail_to($email, $name = '', $options = array(), $default_value = array())
{
  ...
  if (($name == $email) || ($name == _encodeText($email)))
  {
    return content_tag('a', $name, $html_options);
  } else {
    //some low-end mobile phone do not support mailto link.
    //the email address is attached to the name if they are not equal.
    return content_tag('a', $name.'('.$email.')', $html_options);
  }
}
