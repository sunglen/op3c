...
class opToolkit
{
  ...
  */
   ...
   * @return bool true if $string is valid mobile e-mail address and false otherwise.
   ...
   Chinese Mobile Phone Email Address list:
   Format: Number(first 3 digits) + Number(last 8 digits) + @ + Domain
   1. ChinaMobile:
   Number: 134.135.136.137.138.139.150.151.152.157.158.159.187.188.147(DataCard)
   Domain: 139.com
   2. ChinaUnicom: 
   Number: 130.131.132.155.156.185.186
   Domain: wo.com.cn
   3. ChinaTelecom: 
   Number: 133.153.180.189
   Domain: 189.cn

   ATTENTION: isMobileEmailAddress is only used for checking what is NOT a PC mail address.
   See opValidatorPCEmail.class.php for details!
   */
  public static function isMobileEmailAddress($string)
  {
    ...
    /*
      ...
      sfValidatorMobileEmail.class.php.
      DO NOT add an entry in mobile_mail_domain.yml.
    */
    $domain_zh['chinamobile'] = '/139\.com/i';
    $domain_zh['chinaunicom'] = '/wo\.com\.cn/i';
    $domain_zh['chinatelecom'] = '/189\.cn/i';

    $uid_zh['chinamobile'] = '/^(134|135|136|137|138|139|150|151|152|157|158|159|187|188|147)[0-9]{8}$/';
    $uid_zh['chinaunicom'] = '/^(130|131|132|155|156|185|186)[0-9]{8}$/';
    $uid_zh['chinatelecom'] = '/^(133|153|180|189)[0-9]{8}$/';
    
    /*
    if it's a Chinese mobile phone, owning to the address alias problem,
    uid should be a real mobile phone number other than an alphabetical alias.
    i.e. sun.ge@139.com is a PC Mail Address.
    */

    if(preg_match($domain_zh['chinamobile'], $domain)){
      return preg_match($uid_zh['chinamobile'], $uid);
    } else if(preg_match($domain_zh['chinaunicom'], $domain)){
      return preg_match($uid_zh['chinaunicom'], $uid);
    } else if(preg_match($domain_zh['chinatelecom'], $domain)){
      return preg_match($uid_zh['chinatelecom'], $uid);
    } else if(in_array($domain, self::getMobileMailAddressDomains())){
      return true;
    } else {
      return false;
    }
